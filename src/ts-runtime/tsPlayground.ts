...
const runWindowHtmlConsole = "window html console";
// import runWindowHtmlPlain = require("./run-plain.html");
const runWindowHtmlPlain = "window html plain";
...
const _optionsList = Array.prototype.slice
  .call(_options.getElementsByClassName("option"))
  .map((v: Element) => {
...
function ready(): void {
  tsEditor.onDidChangeModelContent(onCodeChange);
    ...
    if (options()[option].hasOwnProperty(input.name)) {
      if (input instanceof HTMLInputElement) {
        if ((input as HTMLInputElement).type === "checkbox") {
          (input as HTMLInputElement).checked = !!defaultOptions[option][
            input.name
          ];
        } else if ((inputs[i] as HTMLInputElement).type === "text") {
          (input as HTMLInputElement).value = `${
            defaultOptions[option][input.name]
          }`;
        }
...
function updateLocalStorage(initial?: boolean): void {
  if (!!window.tsp.options.uiOptions.localStorageBackup) {
    ...
    localStorage.setItem(
      "tsp",
      encode({ options: { uiOptions: { localStorageBackup: false } } })
    );
  }
}
...
function getWindowCode(html?: string): string {
  html =
    html !== void 0
      ? html
      : options().windowOptions.console
      ? runWindowCodeConsole
      : runWindowCodePlain;
